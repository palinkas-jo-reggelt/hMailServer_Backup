...
Function BackupDatabases {
	...
	If ($UseMySQL) {
		...
		If (Test-Path "$BackupTempDir\hMailData\MYSQLDump_*.sql") {
			Get-ChildItem "$BackupTempDir\hMailData" | Where {$_.Extension -match "sql"} | ForEach {
				$OldMySQLDump = $_.Name
				Debug "Deleting old MySQL database dump : $OldMySQLDump"
				Try {
					Remove-Item "$BackupTempDir\hMailData\$OldMySQLDump" -ErrorAction Stop
		...
		[System.IO.FileInfo]$MySQLDumpFile = "$BackupTempDir\hMailData\MYSQLDump_$((Get-Date).ToString('yyyy-MM-dd')).sql"
		Try {
		...
		Try {
			$RoboCopyIDB = & robocopy "$hMSDir\Database" "$BackupTempDir\hMailData\Database" /mir /ndl /r:43200 /np /w:1 | Out-String
			$BackupSuccess++
			Debug $RoboCopyIDB
